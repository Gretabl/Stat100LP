...
joined_df <- joined_df |>
  mutate( country = case_when( grepl("Hong Kong", country, fixed=TRUE) ~ "Hong Kong",
                               ...
                               grepl("Congo", country, fixed=TRUE) ~ "DRC",
                               ...
                               .default = country ))
country_polygons <- gadm( country = joined_df, level = 0, path = getwd() )
...
countries <- unique(joined_df$country)

country_polygons <- gadm(country = countries, level = 0, path = getwd())

country_polygons <- st_as_sf(country_polygons)

country_polygons <- left_join(country_polygons, joined_df, by = c("NAME_0" = "country"))
